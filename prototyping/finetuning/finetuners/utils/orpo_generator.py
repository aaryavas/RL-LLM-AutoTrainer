...
class ORPODataGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            use_vllm: Whether to use vLLM for generation (default: True)
        """
        self.model = None
        self.tokenizer = None
    ...
    def _load_model(self) -> None:
        ...
        if is_adapter:
            ...
            self.tokenizer.padding_side = "left"  # Explicitly set to ensure it sticks
        else:
        ...
        # Explicitly set padding side to ensure it's left (fixes warning)
    ...
    def _load_vllm_model(self) -> None:
        ...
        if is_adapter:
            ...
            logger.info(f"Loading base model for vLLM: {self.base_model_name}")

            # Initialize vLLM with LoRA support
            self.llm = LLM(
                model=self.base_model_name,
                enable_lora=True,
                # max_lora_rank=64,  # Adjust based on your LoRA config if needed, or let vLLM handle defaults
                gpu_memory_utilization=0.6,  # Adjust as needed
                trust_remote_code=True,
            )

            # Define LoRA request
            # We use the model_path as the LoRA path
            self.lora_request = LoRARequest("adapter", 1, self.model_path)
            ...
            # Merged model
            logger.info(f"Loading merged model for vLLM: {self.model_path}")
            self.llm = LLM(
        ...
        # Ensure padding side is left for tokenizer (though vLLM handles batching differently)
        self.tokenizer.padding_side = "left"
        if self.tokenizer.pad_token is None:
            self.tokenizer.pad_token = self.tokenizer.eos_token
    ...
    def generate_rejected_responses(
        ...
        if self.use_vllm and self.llm is not None:
            return self._generate_vllm(prompts, max_new_tokens, temperature, top_p)
        ...
        for i in range(0, len(prompts), batch_size):
            ...
            if (i + batch_size) % 20 == 0:
                logger.info(
                    f"Generated {min(i + batch_size, len(prompts))}/{len(prompts)} responses"
                )
    ...
    def _generate_vllm(
        ...
        logger.info(f"Generating {len(prompts)} rejected responses using vLLM...")
        ...
        for output in outputs:
            generated_responses.append(output.outputs[0].text.strip())
    ...
    def create_orpo_dataset_from_synthetic(
        ...
        # Create ORPO dataset
        orpo_data = []
        for i, (prompt, chosen, rejected) in enumerate(
    ...
    def create_orpo_dataset(
        ...
        if len(df) != len(rejected_df):
            logger.warning(
                f"Input dataframe length ({len(df)}) does not match rejected dataframe length ({len(rejected_df)}). Truncating to minimum length."
            )
